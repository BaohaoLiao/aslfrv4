...
def process(df, fold, data_dir, output_dir):
    ...
    filtered_parquet_df.to_parquet(os.path.join(output_dir, str(fold) + "_parquet.gzip"), compression='gzip')
    df.to_csv(os.path.join(output_dir, str(fold) + "_batch.csv"), index=False)


def main(data_dir: str, output_dir: str, n_split: int=20):
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    df = pd.read_csv(os.path.join(data_dir, "batch.csv"))
    interval = len(df) // n_split
    _ = Parallel(n_jobs=cpu_count())(
        delayed(process)(df[fold*interval:(fold+1)*interval], fold, data_dir, output_dir)
        for fold in range(n_split+1))
