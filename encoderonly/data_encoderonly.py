...
def encode(landmarks, phrase, table):
    phrase = phrase
...
def maybe_concat_samples(sample1, sample2, concat_prob):
    ...
    if tf.random.uniform(()) < concat_prob:
        ...
        phrase = tf.concat([phrase1, [0], phrase2], axis=0)
    else:
...
def load_dataset(
    ...
    ds = ds.padded_batch(
        args.batch_size,
        padding_values=(PAD, args.pad_token_id),
        padded_shapes=(x_padded_shape, [args.max_target_length]),  # TODO: check encoder
