...
def linear_decay(max_value, current_step, total_steps, warmup_ratio=0.1):
    ...
    if current_step <= warmup_steps:
        return tf.cast(current_step, dtype=tf.float32) * max_value / warmup_steps
    else:
        return max_value - max_value * tf.cast((current_step - warmup_steps), dtype=tf.float32) / (total_steps - warmup_steps)
...
def apply_mask(sample, mask_prob, mask_token_id, random_token_prob, current_step, total_steps):
    ...
    if mask_prob > 0.:
        ...
        mask = tf.random.uniform(shape=tf.shape(mask_range), dtype=tf.float32) < mask_prob
        ...
        masked_char_ids = tf.tensor_scatter_nd_update(masked_char_ids, tf.expand_dims(mask_indices, axis=1),
                                                      mask_values)
        random_replace = mask & (tf.random.uniform(shape=tf.shape(mask), dtype=tf.float32) < random_token_prob)
        random_indices = tf.boolean_mask(mask_range, random_replace)
        random_char_ids = tf.random.uniform(shape=tf.shape(random_indices), minval=0, maxval=59, dtype=tf.int32)
        masked_char_ids = tf.tensor_scatter_nd_update(masked_char_ids, tf.expand_dims(random_indices, axis=1),
                                                      random_char_ids)
...
def preprocess(x, y, args, augment, max_source_length):
