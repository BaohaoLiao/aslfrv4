...
class Conv1DBlockv2(tf.keras.layers.Layer):
    def __init__(self, d_model, conv_dim, kernel_size, dilation_rate, dropout, activation, **kwargs):
        super(Conv1DBlockv2, self).__init__(**kwargs)
        self.dense1 = tf.keras.layers.Dense(conv_dim, use_bias=True, activation=activation, name="dense1")
        self.causal_conv = CausalDWConv1D(
            kernel_size=kernel_size, dilation_rate=dilation_rate, use_bias=False, name="causal_conv")
        self.norm = tf.keras.layers.LayerNormalization(epsilon=1e-6, name="ln")
...
class CNNDecoderLayer(tf.keras.layers.Layer):
    def __init__(self, d_model, conv_dim, kernel_size, dilation_rate, attn_dropout, hidden_dropout, activation,
                 num_heads, mlp_dim, prenorm, **kwargs):
        super(CNNDecoderLayer, self).__init__(**kwargs)
        self.convs = [Conv1DBlockv2(
            d_model=d_model,
            conv_dim=conv_dim,
            kernel_size=kernel_size,
            dilation_rate=dilation_rate,
            dropout=hidden_dropout,
            activation=activation,
            name=f"conv{i}") for i in range(2)]
    ...
    def call(self, x, encoder_out, attention_mask, encoder_attention_mask, training):
        ...
        x = self.transformer_decoder_layer(
            x,
...
class CNNDecoder(tf.keras.layers.Layer):
    def __init__(self, vocab_size, num_layers, d_model, max_target_length, learnable_position, num_heads, mlp_dim,
        emb_dropout, attn_dropout, hidden_dropout, pad_token_id, prenorm, activation, conv_dim, kernel_size,
...
class CNNEncoderCNNDecoder(tf.keras.Model):
    def __init__(
        self,
        ...
        prenorm: bool,
        activation: str):
        super(CNNEncoderCNNDecoder, self).__init__()
        ...
        self.decoder = CNNDecoder(
            ...
            dilation_rate=decoder_dilation_rate,
            name="decoder")
